...
all_post_test_cross_hanam <- load_mcmc_post(all_models_hanam[[1]],
    fit_place = "local",
...
save(post_cross, file = here::here("outputs", "hanam", "fits", "dis_dep", "cross_sec", "ln_post_cross.Rdata"))
...
for (p in 1:1) { 
    ...
    B <- round(L / 2)
    chain_list <- all_post_test_cross_hanam$theta_list_chain
    pars_plot <- setdiff(all_models_hanam[[1]]$other$pars_plot, "tau_vac" )
    all_post_test_cross_trim_hanam[[p]] <- chain_list %>% map(~(.x[B:L, pars_plot] %>% as.data.frame))
    samp_no_list[[p]] <- chain_list %>% map(~.x[B:L, c("sampno")])
}
...
for (p in 1:1) { 
    ...
    inf_hist <- all_post_test_cross_hanam$inf_list_chains
    all_post_test_cross_trim_inf_hist_hanam[[p]] <- inf_hist %>%
        map(~(.x %>% filter(sampno == 130001) %>% as.data.frame %>%
        dplyr::rename(yr = i, id = j)))
}

post_cross <- list(post_raw = all_post_test_cross_hanam, post_trim = all_post_test_cross_trim_hanam, inf_hist_trim = all_post_test_cross_trim_inf_hist_hanam)
save(post_cross, file = here::here("outputs", "hanam", "fits", "dis_dep", "full", "LN_post_full.Rdata"))
...
plot_traces(all_post_test_cross_hanam, all_models_hanam[[2]], file = "dis_dep/full")
plot_histogram(all_post_test_cross_hanam, all_models_hanam[[2]], file = "dis_dep/full")
plot_titre_post(all_post_test_cross_hanam, all_models_hanam[[2]], file = "dis_dep/full")
