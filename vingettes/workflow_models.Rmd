...
post_best %>% apply(2, mean) %>% as.data.frame %>% t
...
plot_traces(all_post_test_full[[4]], all_models_hcw_pre_full[[4]],  file = "dis_dep/full")

list_chains <- all_post_test_full[[4]]$theta_list_chains
j <- 1
L <- nrow(list_chains[[4]])
B <- round(L / 2)
list_chains1 <- lapply(list_chains, 
    function(x) x[B:L, 
        c(all_models_hcw_pre_full[[4]]$other$pars_plot, "total_infections", "lnlike", "prior_prob")])
...
```{r}
# + mean(postplots_long[["wane_vac"]]
postplots_long[["mu_short_vac"]] * (1.0 - ((1 / 12 + mean(postplots_long[["wane_vac"]])) * 12 * 1))

postplots_long <- all_post_test_full_trim[[1]] %>% bind_rows
dec <- 1:12 %>% map(~
    postplots_long[["mu"]] * postplots_long[["mu_vac"]] + (postplots_long[["mu_short_vac"]] * (1.0 - (((postplots_long[["mu_short_vac"]] * 1) / 12 + postplots_long[["wane_vac"]]) * .x * 1)))
) 
