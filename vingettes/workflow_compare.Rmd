...
vrius_t <- titre_dat %>% pull(virus) %>% unique
virus_v <- vaccination_histories %>% pull(virus) %>% unique

check_strains <- function(strain_isolation_times, titre_dat, vaccination_histories) {
    vrius_t <- titre_dat %>% pull(virus) %>% unique
    virus_v <- vaccination_histories %>% pull(virus) %>% unique
    sum(!(vrius_t %in% strain_isolation_times))
    sum(!(virus_v %in% strain_isolation_times))
}

individuals_id <- vaccination_histories %>% pull(individual) %>% unique 
vaccination_histories %>%
    complete(nesting(virus = strain_isolation_times, time = strain_isolation_times), individual = individuals_id, 
...
chain <- as.data.frame(all_post_test_full[[1]]$theta_chain) %>% filter(chain_no == 1)
infection_histories <- all_post_test_full[[1]]$inf_chain %>% filter(chain_no == 1)
titre_dat <- all_models_hcw_pre_full[[1]]$create_post$titre_data 
vaccination_histories <- all_models_hcw_pre_full[[1]]$create_post$vaccination_histories

antigenic_map <- all_models_hcw_pre_full[[1]]$create_post$antigenic_map
strain_isolation_times <-  all_models_hcw_pre_full[[1]]$create_post$strain_isolation_times
par_tab <- all_models_hcw_pre_full[[1]]$create_post$par_tab

vaccination_histories <- all_models_hcw_pre_full[[1]]$create_post$vac_history
custom_ab_kin_func <- all_models_hcw_pre_full[[1]]$custom_ab_kin_func
custom_ab_kin_func_1 <- all_models_hcw_pre_full[[1]]$custom_ab_kin_func

custom_antigenic_maps_func <- all_models_hcw_pre_full[[1]]$custom_antigenic_maps_func
