        ...
        if(sampling_time > infection_times[x_inf]) {
            ...
            for(int k = 0; k < n_titres; ++k){
              ...
              predicted_titres[tmp_titre_index + k] += seniority *
                (mu * antigenic_map_long[index]) * (1 + (mu_short - 1) * (antigenic_map_short[index] * wane_amount));
            }
        }
        ++x_inf;
      }
          ...
          if(sampling_time > vaccination_times[x_vac]) {
            ...
            for(int k = 0; k < n_titres; ++k){
              ...
              double titre_incr_amount = (seniority) * (mu * mu_vac * antigenic_map_long_vac[index]) *
                (1 + (mu_short_vac - 1) * (antigenic_map_short_vac[index] * wane_amount_vac));
